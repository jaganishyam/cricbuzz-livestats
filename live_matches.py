...
def render():
    ...
    for m in shown:
        with st.container(border=True):
            ...
            if score:
                # cricbuzz keys the score dict "team1Score"/"team2Score" - not
                # actual team names - so map those back to the real names we
                # already pulled out of matchInfo instead of printing the raw key
                team_names = {"team1Score": m["team1"], "team2Score": m["team2"]}
                cols = st.columns(len(score))
                for i, (key, s) in enumerate(score.items()):
                    innings = s.get("inngs1", {}) if isinstance(s, dict) else {}
                    if innings:
                        label = team_names.get(key, key)
                        cols[i].metric(label, f"{innings.get('runs','-')}/{innings.get('wickets','-')}",
